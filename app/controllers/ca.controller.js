...
function preApply(req, res, next) {
	...
	if (!cert.name || !cert.idnumber || !cert.certnumber || !req.body.studNumber) {
		return next(new Error('not full info'));
	}
				...
				CA.findOne({
					...
					.exec(function(err, ca) {
						function makeReport(ca) {
							...
							// overwrite
							report._id = ca._id;
							report.studNumber = ca.studNumber;
							return report;
						}
...
function search(req, res, next) {
	...
	var criteria = _.pick(req.query, ['major', '_id', "studNumber"]);
	var certCriteria = _.pick(req.query, fields);


	if (fuzzy) {
		criteria = toFuzzySearch(criteria);
		certCriteria = toFuzzySearch(certCriteria);
	}

	var certs;
	// due to my poor data model design, I have to do such things:
	// in other places we should use workType instead of worktype, the latter is a debris
	// this is a custom method added by me:
